...
class CStartCompositionEditSession : public CEditSession {
 public:
  CStartCompositionEditSession(com_ptr<BangkeTSF> pTextService,
...
void BangkeTSF::_StartComposition(com_ptr<ITfContext> pContext,
                                  BOOL fCUASWorkaroundEnabled) {
...
class CEndCompositionEditSession : public CEditSession {
 public:
  CEndCompositionEditSession(com_ptr<BangkeTSF> pTextService,
                             com_ptr<ITfContext> pContext,
                             com_ptr<ITfComposition> pComposition,
                             BOOL clear = TRUE)
...
void BangkeTSF::_EndComposition(com_ptr<ITfContext> pContext,
                                BOOL clear,
                                BOOL endUI) {
...
class CGetTextExtentEditSession : public CEditSession {
 public:
  CGetTextExtentEditSession(com_ptr<BangkeTSF> pTextService,
                            com_ptr<ITfContext> pContext,
...
BOOL BangkeTSF::_UpdateCompositionWindow(com_ptr<ITfContext> pContext) {
  com_ptr<ITfContextView> pContextView;
...
void BangkeTSF::_SetCompositionPosition(const RECT& rc) {
...
class CInlinePreeditEditSession : public CEditSession {
 public:
  CInlinePreeditEditSession(com_ptr<BangkeTSF> pTextService,
                            com_ptr<ITfContext> pContext,
                            com_ptr<ITfComposition> pComposition,
                            const std::shared_ptr<bangke::Context> context)
      : CEditSession(pTextService, pContext),
        _pComposition(pComposition),
        _context(context) {}
 ...
 private:
  com_ptr<ITfComposition> _pComposition;
  const std::shared_ptr<bangke::Context> _context;
};
...
STDMETHODIMP CInlinePreeditEditSession::DoEditSession(TfEditCookie ec) {
  ...
  for (size_t i = 0; i < _context->preedit.attributes.size(); i++) {
    if (_context->preedit.attributes.at(i).type == bangke::HIGHLIGHTED) {
...
BOOL BangkeTSF::_ShowInlinePreedit(
    com_ptr<ITfContext> pContext,
    const std::shared_ptr<bangke::Context> context) {
...
class CInsertTextEditSession : public CEditSession {
 public:
  CInsertTextEditSession(com_ptr<BangkeTSF> pTextService,
                         com_ptr<ITfContext> pContext,
                         com_ptr<ITfComposition> pComposition,
...
BOOL BangkeTSF::_InsertText(com_ptr<ITfContext> pContext,
                            const std::wstring& text) {
...
void BangkeTSF::_UpdateComposition(com_ptr<ITfContext> pContext) {
  HRESULT hr;
...
/* Composition State */
STDMETHODIMP BangkeTSF::OnCompositionTerminated(TfEditCookie ecWrite,
...
void BangkeTSF::_AbortComposition(bool clear) {
...
void BangkeTSF::_FinalizeComposition() {
  _pComposition = nullptr;
}

void BangkeTSF::_SetComposition(com_ptr<ITfComposition> pComposition) {
  _pComposition = pComposition;
}

BOOL BangkeTSF::_IsComposing() {
  return _pComposition != NULL;
}

BOOL BangkeTSF::_IsCurrentComposition(ITfComposition* pComposition) {
