...
BOOL BangkeTSF::_IsKeyboardDisabled() {
...
BOOL BangkeTSF::_IsKeyboardOpen() {
...
HRESULT BangkeTSF::_SetKeyboardOpen(BOOL fOpen) {
  HRESULT hr = E_FAIL;
  com_ptr<ITfCompartmentMgr> pCompMgr;
...
HRESULT BangkeTSF::_GetCompartmentDWORD(DWORD& value, const GUID guid) {
  HRESULT hr = E_FAIL;
...
HRESULT BangkeTSF::_SetCompartmentDWORD(const DWORD& value, const GUID guid) {
  HRESULT hr = S_OK;
  com_ptr<ITfCompartmentMgr> pComMgr;
  if (_pThreadMgr->QueryInterface(&pComMgr) == S_OK) {
    ITfCompartment* pCompartment;
...
BOOL BangkeTSF::_InitCompartment() {
  using namespace std::placeholders;

  auto callback = std::bind(&BangkeTSF::_HandleCompartment, this, _1);
...
void BangkeTSF::_UninitCompartment() {
  if (_pKeyboardCompartmentSink) {
...
HRESULT BangkeTSF::_HandleCompartment(REFGUID guidCompartment) {
  if (IsEqualGUID(guidCompartment, GUID_COMPARTMENT_KEYBOARD_OPENCLOSE)) {
    if (_isToOpenClose) {
      ...
      _SetAsciiMode(_status.ascii_mode);
    ...
    BOOL isOpen = _IsKeyboardOpen();
    if (isOpen) {
      bangke::ResponseParser parser(NULL, NULL, &_status, NULL,
