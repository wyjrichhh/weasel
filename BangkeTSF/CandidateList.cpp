...
CCandidateList::CCandidateList(com_ptr<BangkeTSF> pTextService)
...
void CCandidateList::UpdateUI(const Context& ctx, const Status& status) {
  if (_ui->style().inline_preedit) {
    _ui->style().client_caps |= bangke::INLINE_PREEDIT_CAPABLE;
  } else {
    _ui->style().client_caps &= ~bangke::INLINE_PREEDIT_CAPABLE;
  }
...
void BangkeTSF::_UpdateUI(const Context& ctx, const Status& status) {
  _cand->UpdateUI(ctx, status);
}

void BangkeTSF::_StartUI() {
  _cand->StartUI();
}

void BangkeTSF::_EndUI() {
  _cand->EndUI();
}

void BangkeTSF::_ShowUI() {
  _cand->Show(TRUE);
}

void BangkeTSF::_HideUI() {
  _cand->Show(FALSE);
}

com_ptr<ITfContext> BangkeTSF::_GetUIContextDocument() {
  return _cand->GetContextDocument();
}

void BangkeTSF::_DeleteCandidateList() {
  _cand->Destroy();
}

void BangkeTSF::_SelectCandidateOnCurrentPage(size_t index) {
...
void BangkeTSF::_HandleMousePageEvent(bool* const nextPage,
...
void BangkeTSF::_HandleMouseHoverEvent(const size_t index) {
...
void BangkeTSF::HandleUICallback(size_t* const sel,
                                 size_t* const hov,
                                 bool* const next,
