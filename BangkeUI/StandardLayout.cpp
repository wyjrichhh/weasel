...
void bangke::StandardLayout::GetTextSizeDW(
...
CSize StandardLayout::GetPreeditSize(CDCHandle dc,
                                     const bangke::Text& text,
                                     ComPtr<IDWriteTextFormat1> pTextFormat,
                                     PDWR pDWR) {
  const std::wstring& preedit = text.str;
  const std::vector<bangke::TextAttribute>& attrs = text.attributes;
  CSize size(0, 0);
  if (!preedit.empty()) {
    bangke::TextRange range;
    for (size_t j = 0; j < attrs.size(); ++j)
      if (attrs[j].type == bangke::HIGHLIGHTED)
...
bool bangke::StandardLayout::_IsHighlightOverCandidateWindow(CRect& rc,
                                                             CDCHandle& dc) {
...
void bangke::StandardLayout::_PrepareRoundInfo(CDCHandle& dc) {
...
void StandardLayout::UpdateStatusIconLayout(int* width, int* height) {
  ...
  // and the status icon rule 3. status icon is right aligned in the panel,
